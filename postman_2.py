...
class Table_Queries:
	...
	def aggregate_query(self):
		...
		data_ = pd.read_sql(query, self.conn).to_csv('E:\prog\aggregated_products.csv')
		...
		try:
			...
			self._cursor.execute(''' CREATE TABLE Aggregated_Products(name nvarchar(50), product int)''')
		...
		self._cursor.execute("BULK INSERT " + 'JAYANTA.dbo.Aggregated_Products' + " FROM '" + 'E:\prog\aggregated_products.csv' + "' WITH (FORMAT = 'CSV', FIRSTROW = 2)")
			...
			if len(v) >= 1 :
				...
				for i in v :
					...
					if choice == 'yes':
						...
						_data = self.data.to_csv(input("Enter the file name to be saved:"))

						data_link =r'E:\prog\updated_products.csv'
